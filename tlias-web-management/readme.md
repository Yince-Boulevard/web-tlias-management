...
### AOP编程（面向切面编程Aspect-Oriented Programming）
...
发现学员查询有bug，发现学员查询参数实体类的 班级id名字错了，把classId改成clazzId并且一并修改Mapper.xml中的字段名后修改正确
...
3. @After: 后置通知，在目标方法后被执行（无论是否异常）
4. @AfterReturning: 返回后通知，在目标方法后被执行（有异常不执行）
5. @AfterThrowing: 异常后通知，发生异常后才执行

> @Around需要自己调用ProceedingJointPoint.proceed() 来让原始方法执行，其他通知不需要考虑目标方法执行
>
> @Around方法的返回值必须设置为Object来接受原始方法的返回值

#### 通知的执行顺序

1. 多个AOP默认按照字母排序先后执行
2. 在切面类上 `@Order(数字)` 数字越小先执行
...
> 1. 基于签名
>    exection(访问修饰符? 返回值 包名.类名.?方法名(方法参数) throws 异常?)
...
> 获取目标方法的相关信息（目标对象，目标对象对应的类，目标方法，目标方法的参数）
>
> 针对@Around注解，通过ProceedingJoinPoint
>
> 针对其他注解，通过JoinPoint
...
在Token的拦截器中解析完当前登录员工ID，将其存入ThreadLocal（用完之后删除）

在AOP程序中，从ThreadLocal中获取当前登录员工ID
...
program arguments(命令行参数) > vm options(Java系统属性) > .properties > .yml > .ymal
...
| session         | 每个会话范围内创建新的实例                    |
| application     | 每个应用范围内创建新的实例                    |

> ❤️ `@Scope("prototype")` 指定bean的作用域
>
> 😕 单例: bean默认在项目启动时创建
> `@Lazy`将单例bean延迟到第一次使用的时候创建
>
> 👀️ 多例：bean每次使用的时候创建一次

##### 应用场景

单例：无状态的bean（不保存数据的），总是操作的同一个对象

多例：有状态的bean(需要保存数据)，每次操作不同的对象
